...
{
    thisCyclePredictedDirRad = MathTools::normalize(dirRad + (ws->getMotors()->predictInertiaRight(act->getrPower()) - ws->getMotors()->predictInertiaLeft(act->getlPower())) / DIAMETER);
    thisCyclePredictedDirDeg = MathTools::rad2deg(thisCyclePredictedDirRad);

    thisCyclePredictedPosX = posX + ((ws->getMotors()->predictInertiaRight(act->getrPower()) + ws->getMotors()->predictInertiaLeft(act->getlPower())) * cos(dirRad)) / 2;
    thisCyclePredictedPosY = posY + ((ws->getMotors()->predictInertiaRight(act->getrPower()) + ws->getMotors()->predictInertiaLeft(act->getlPower())) * sin(dirRad)) / 2;
}
