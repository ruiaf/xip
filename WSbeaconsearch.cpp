...
void WSbeaconsearch::update()
{
  ...
  for (unsigned i=0;i<search_area.size();i++)
    {
      if (search_area[i].s == UNREACHABLE) continue;
      
      if (outOfBounds(i))
	{ search_area[i].s = UNREACHABLE; continue; }
      
      if (inArea(i))
	{ search_area[i].s = VISITED; continue; }
      
      if (search_area[i].s >= HIGH_PROB_SEEN) continue;
      ...
      if (dist<4) new_s = HIGH_PROB_SEEN;
      else if (dist<8) new_s = MID_PROB_SEEN;
      else new_s = LOW_PROB_SEEN;

    
      DiscretePosition mypos=DiscretePosition(Position(search_area[i].x,search_area[i].y));
      if (new_s > search_area[i].s && ws->getMaze()->goodToGoWithMouse(mypos.getX(),mypos.getY())<3) search_area[i].s = new_s;
    }
...
{
  if (ws->getPosition()->getX() >= search_area[i].x-AREA_SIZE &&
      ws->getPosition()->getX() <= search_area[i].x+2*AREA_SIZE &&
      ws->getPosition()->getY() >= search_area[i].y-AREA_SIZE &&
      ws->getPosition()->getY() <= search_area[i].y+2*AREA_SIZE)
    return true;
