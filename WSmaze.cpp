...
WSmaze::WSmaze(WorldState *w)
{
	...
	printf("A calcular tabelas"); fflush(stdout);
	createRobotMatrix();
	createSensor();
...
{ 
	...
	//
	if (!ws->getSensors()->hasIRSensors()) return;

	// passagens estreitas, feio :(
	if (ws->getSensors()->getLeftSensor() > 0.5  && ws->getSensors()->getCenterLeftSensor() < 2 ||
			ws->getSensors()->getLeftSensor() < 2  && ws->getSensors()->getCenterLeftSensor() > 0.5)
	{
	...
	if (ws->getSensors()->getCenterRightSensor() > 0.5 &&  ws->getSensors()->getRightSensor() < 2 ||
			ws->getSensors()->getCenterRightSensor() < 2 &&  ws->getSensors()->getRightSensor() > 0.5)
	{
...
void WSmaze::setMazePos(double x, double y, char val, double certeza)
{
	...
	if (certeza>0.95) peso = 50;
	else if (certeza > 0.6)
		if (val<=8 && val>=0 ) peso = 5;
		else peso = 2;
	else
		if (val==0) peso = 5;
		else if (val>0 && val<2 ) peso = 2;
		else peso = 1;
...
void WSmaze::createSensor(void)
{
	...
	for (int dir=0;dir<360;dir++)
	{
		...
		for (int sensordist=0;sensordist<=MAZE_DEF;sensordist++)
		{
			if (sensordist==0)
				sensor_val = 100;
			else sensor_val=1/(((double)sensordist)/MAZE_DEF);
			if (sensor_val>100)
				sensor_val=100;
				...
				for (j=0;j<3*MAZE_DEF;j++)
				{
					...
					if (MathTools::angle_between(ang,alpha,beta) && dist <= sensordist+0.5*MAZE_DEF)
					{
						...
						if (dist_max_parede<=0)
							sensor_val_max=100;
						else sensor_val_max = ((double)1)/(dist_max_parede/MAZE_DEF);
						if (sensor_val_max>100) sensor_val_max=100;


						if (dist==0)
							cur_sensorval=100;
						else cur_sensorval = 1/(dist/MAZE_DEF);
						if (cur_sensorval>100) cur_sensorval=100;
					...
					else sensor[dir][sensordist][i][j]=-1;
				}
		}
	}
	...
	for (int dir=0;dir<360;dir++)
	{
		...
		for (int sensor_val=0; sensor_val<10;sensor_val++)
		{
			if (sensor_val==0)
				sensordist = 20;
			else sensordist=((double)1)/(((double)sensor_val)/10);

			sensordist*=MAZE_DEF;

			if (sensor_val<3)
				sensor_long[dir][sensor_val].size=(int) 100;
			else sensor_long[dir][sensor_val].size= (int) sensordist*2+10;
			...
			for (i=0;i<sensor_long[dir][sensor_val].size;i++)
				for (j=0;j<sensor_long[dir][sensor_val].size;j++)
				{
					...
					else sensor_long[dir][sensor_val].mat[i][j]=-1;
				}
		}
	}
}
...
void WSmaze::erode(char newmatrix[2*MAZE_DEF*LAB_X][2*MAZE_DEF*LAB_Y], char oldmatrix[2*MAZE_DEF*LAB_X][2*MAZE_DEF*LAB_Y])
{
	...
	for (i=minx;i<maxx;i++)
		for (j=miny;j<maxy;j++)
		{
			if (oldmatrix[i][j]<0) newmatrix[i][j]=-1;
			else if ( (oldmatrix[i-1][j-1] > PROB_CONSID_WALL || oldmatrix[i-1][j-1] < 0) &&
					(oldmatrix[i-1][j] > PROB_CONSID_WALL || oldmatrix[i-1][j] <0) &&
					(oldmatrix[i-1][j+1] > PROB_CONSID_WALL || oldmatrix[i-1][j+1] <0) &&
					(oldmatrix[i][j-1] > PROB_CONSID_WALL || oldmatrix[i][j-1]<0) &&
					(oldmatrix[i][j] > PROB_CONSID_WALL || oldmatrix[i][j]<0) &&
					(oldmatrix[i][j+1] > PROB_CONSID_WALL || oldmatrix[i][j+1]<0) &&
					(oldmatrix[i+1][j-1] > PROB_CONSID_WALL || oldmatrix[i+1][j-1]<0) &&
					(oldmatrix[i+1][j] > PROB_CONSID_WALL || oldmatrix[i+1][j]<0) &&
					(oldmatrix[i+1][j+1] > PROB_CONSID_WALL || oldmatrix[i+1][j+1]<0) )
	...
	for (i=minx;i<maxx;i++)
		for (j=miny;j<maxy;j++)
		{
			if (oldmatrix[i][j]<0) newmatrix[i][j]=-1;
					...
					(oldmatrix[i-1][j] > PROB_CONSID_WALL ) &&
					(oldmatrix[i-1][j+1] > PROB_CONSID_WALL) &&
					(oldmatrix[i][j-1] > PROB_CONSID_WALL) &&
					(oldmatrix[i][j] > PROB_CONSID_WALL) &&
					(oldmatrix[i][j+1] > PROB_CONSID_WALL) &&
					(oldmatrix[i+1][j-1] > PROB_CONSID_WALL) &&
					(oldmatrix[i+1][j] > PROB_CONSID_WALL) &&
					(oldmatrix[i+1][j+1] > PROB_CONSID_WALL))
				newmatrix[i][j] = oldmatrix[i][j];
			else if (oldmatrix[i][j] > PROB_CONSID_WALL) newmatrix[i][j] = -1;
		...
		int dx,int dy)
{
	...
	for (i=-(SMALL_LAB_X-1)*MAZE_DEF;i<(SMALL_LAB_X-1)*MAZE_DEF;i++)
		for (j=-(SMALL_LAB_Y-1)*MAZE_DEF;j<(SMALL_LAB_Y-1)*MAZE_DEF;j++)
		{
			...
			if (ssamples[slabi][slabj]<5 ||
					samples[labi][labj]<70)
				continue;
...
void WSmaze::decreaseSmallMazeSamples()
{
	...
	for (int i=minx;i<maxx;i++)
		for (int j=miny;j<maxy;j++)
		{
			if ((i<(myposx-SMALL_LAB_X*MAZE_DEF) || i>(myposx+SMALL_LAB_X*MAZE_DEF)) ||
					(j<(myposy-SMALL_LAB_Y*MAZE_DEF) || j>(myposy+SMALL_LAB_Y*MAZE_DEF)))
			{
				Smallsamples[i][j]=0;
				Smallmaze[i][j]=-1;
			}
...
{  
	...
	if (esq<val && dir-val>POS_MOVE_MARGIN) { movx=-0.1/4; }
	else if (dir<val && esq-val>POS_MOVE_MARGIN) { movx=0.1/4; }

	if (cima<val && baixo-val>POS_MOVE_MARGIN){ movy=0.1/4; }
	else if (baixo<val && cima-val>POS_MOVE_MARGIN){ movy=-0.1/4; }
