...
Action* Plan::wayTo(const Position& originPos, const Position& destinationPos)
{
    ...
    node = pq->deleteMin();
    if (node->isFinal())
        goto skipWayTo; // big hack! corrigir isto!
        ...
        for (int i = -JUMP; i <= JUMP; i += JUMP)
            for (int j = -JUMP; j <= JUMP; j += JUMP) {
                if (!(i == 0 && j == 0)) {
                    ...
                    if (ws->getMaze()->CanGoWithMouse(newPos) && !sucessor->isExpansioned() && ((cost_inc = sqrt((double)(i * i + j * j)) + ws->getMaze()->goodToGoWithMouse(newPos)), !sucessor->beenVisited() || sucessor->getValue() > node->getValue() + cost_inc)) {
                        double turn_overhead = (fabs(MathTools::calc_angle(
                                                    node->getPosition().getX(),
                                                    node->getPosition().getY(),
                                                    sucessor->getPosition().getX(),
                                                    sucessor->getPosition().getY()))
                                                   < 91)
                            / (5);
    ...
    while (true) {
        ...
        } while (!ws->getMaze()->validToGoWithMouse(node->getPosition()) || node->isExpansioned());

        node->setExpansioned();

        if (node->isFinal())
            break; // consegui chegar ao destino! :D
...
skipWayTo:
    ...
    if (optimizedRouteToDestination.size() > 1) // se ainda não estivermos no destino..
    {
        if (ws->getMaze()->goodToGoWithMouse(optimizedRouteToDestination[1]))
            ...
            return motor->goToXY(optimizedRouteToDestination[1].getPosition());
    }
...
Action* Plan::wayThroughKnownPlacesTo(const Position& originPos, const Position& destinationPos)
{
    ...
    node = pq->deleteMin();
    if (node->isFinal())
        goto skipWayTKPTo; // big hack! corrigir isto!
            ...
            for (int j = -JUMP; j <= JUMP; j += JUMP) {
                if (!(i == 0 && j == 0)) {
                    DiscretePosition newPos(node->getPosition().getX() + i,
                        node->getPosition().getY() + j);
                    sucessor = getPath(newPos);
                    if (ws->getMaze()->CanGoWithMouse(newPos) && ws->getMaze()->isKnown(newPos) && !sucessor->isExpansioned() && ((cost_inc = sqrt((double)(i * i + j * j)) + ws->getMaze()->goodToGoWithMouse(newPos)), !sucessor->beenVisited() || sucessor->getValue() > node->getValue() + cost_inc)) {
                        sucessor->update(node, cost_inc);
                        pq->insert(sucessor);
                    }
                }
            ...
            for (int j = -JUMP; j <= JUMP; j += JUMP) {
                if (!(i == 0 && j == 0)) {
                    DiscretePosition newPos(node->getPosition().getX() + i,
                        node->getPosition().getY() + j);
                    sucessor = getPath(newPos);
                    if (ws->getMaze()->CanGoWithMouse(newPos) && !sucessor->isExpansioned() && (cost_inc = (sqrt((double)(i * i + j * j)) + ws->getMaze()->goodToGoWithMouse(newPos)) * (ws->getMaze()->isKnown(newPos) ? 5 : 1), !sucessor->beenVisited() || sucessor->getValue() > node->getValue() + cost_inc)) {
                        sucessor->update(node, cost_inc);
                        pq->insert(sucessor);
                    }
                }
...
int Plan::getNextOptimizedPoint()
{
    ...
    while (optimizedIndex < route_size && tryWayToPoint(optimizedIndex))
        optimizedIndex++;
