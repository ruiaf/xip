	...
	if (!max_rot && fabs (trueMovement) > 0.01)
	  {
	    ...
	    trueMovement = -0.15*factor_normalized_cl*factor_normalized_cr +
		    trueMovement*(1-factor_normalized_cl*factor_normalized_cr);
	    ...
	    if (factor_normalized_cl>0.8 ||
		factor_normalized_cr>0.8)
	      wasLastActionCorrected = true;


	    
	    ///////////////////// side obstacle
	    trueHeading = (((-(2.0-wcr)*trueHeading*factor_normalized_l+
					    trueHeading*(1-factor_normalized_l))+
				    (-(2.0-wcl)*trueHeading*factor_normalized_r+
				     trueHeading*(1-factor_normalized_r))))/2;
...
void MotorController::updateChangedActions()
{
	...
	while (!listChangedActions.empty())
	{
		...
		if (c.time+TIME_MOUSE_AHEAD<ws->getTime() ||
				MathTools::distance(c.x,c.y,ws->getPosition()->getX(),ws->getPosition()->getY()) > 0.5)
		{
